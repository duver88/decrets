...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
    ...
    <form action="{{ route('concepts.update', $concept->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <select name="concept_type_id" id="concept_type_id" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                ...
                @foreach($conceptTypes as $type)
                    <option value="{{ $type->id }}" {{ old('concept_type_id', $concept->concept_type_id) == $type->id ? 'selected' : '' }}>{{ $type->nombre }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="concept_theme_id" id="concept_theme_id" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                ...
                @foreach($themes as $theme)
                    <option value="{{ $theme->id }}" {{ old('concept_theme_id', $concept->concept_theme_id) == $theme->id ? 'selected' : '' }}>{{ $theme->nombre }}</option>
                @endforeach
            </select>
        ...
        <div class="flex justify-end mt-6">
            ...
            <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px<button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-md transition-colors">
                Guardar Cambios
            </button>
        </div>
    </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Función para cargar temas según el tipo seleccionado
            ...
            fetch(`/concepts/themes/${typeId}`)
                ...
                .then(data => {
                    ...
                    data.forEach(theme => {
                        ...
                        if (theme.id == "{{ old('concept_theme_id', $concept->concept_theme_id) }}") {
